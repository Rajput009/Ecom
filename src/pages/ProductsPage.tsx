...
export function ProductsPage() {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'Graphics Cards': return Monitor;
      case 'Processors': return Cpu;
      case 'Memory': return MemoryStick;
      case 'Storage': return HardDrive;
      case 'Cooling': return Fan;
      case 'Power Supplies': return Zap;
      default: return Cpu;
    }
  };
    ...
    <div className="min-h-screen bg-[#0a0a0b] pt-20">
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            ...
            <p className="text-[#71717a] mt-1 font-mono text-sm">
              ...
              {searchQuery && (
                <button 
                  ...
                  className="ml-3 text-[#3b82f6] hover:text-[#60a5fa] underline"
                >
                  Clear Search
                </button>
              )}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="md:hidden flex items-center gap-2 px-4 py-2 bg-[#18181b] border border-[#27272a] rounded-lg text-sm text-white font-mono hover:border-[#3b82f6] transition-colors"
            >
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none px-4 py-2 pr-10 bg-[#18181b] border border-[#27272a] rounded-lg text-sm text-white font-mono focus:outline-none focus:border-[#3b82f6] hover:border-[#3f3f46] transition-colors cursor-pointer"
              >
                {sortOptions.map(opt => (
        ...
        <div className="flex gap-6">
          {/* Sidebar Filters */}
          <aside className={cn(
            showFilters ? 'fixed inset-0 z-50 bg-black/80 md:relative md:bg-transparent' : 'hidden',
            'md:block'
          )}>
            <div className={cn(
              showFilters ? 'absolute right-0 top-0 h-full w-80 bg-[#111113] border-l border-[#27272a] p-6 overflow-y-auto' : '',
              'md:relative md:w-64 md:p-0 md:bg-transparent md:border-0'
            )}>
              {/* Mobile Close */}
              {showFilters && (
                <div className="flex items-center justify-between mb-6 md:hidden">
                  ...
                  <button 
                    ...
                    className="w-8 h-8 flex items-center justify-center rounded-lg bg-[#18181b] text-[#71717a] hover:text-white"
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                  ...
                  <div className="space-y-1">
                    {priceRanges.map(range => (
                      <button
                        ...
                        className={cn(
                          "w-full text-left px-3 py-2.5 rounded-lg text-sm transition-all font-mono",
            ...
            {viewMode === 'grid' ? (
              ...
              <div className="space-y-4">
                {filteredProducts.map(product => (
                  <div
                    key={product.id}
                    className="flex gap-4 bg-[#111113] border border-[#27272a] rounded-xl p-4 hover:border-[#3b82f6] transition-all cursor-pointer group"
                    onClick={() => setSelectedProduct(product)}
                  >
                    <div className="w-32 h-32 bg-[#18181b] rounded-lg overflow-hidden shrink-0">
                      <img
                        src={product.image}
                        alt={product.name}
                      ...
                      {product.specs && (
                        <div className="flex flex-wrap gap-2 mb-3">
                          {product.specs.slice(0, 3).map((spec) => (
                            <span key={spec} className="text-[10px] font-mono text-[#71717a] bg-[#18181b] px-2 py-1 rounded">
                              {spec}
                            </span>
                          ))}
                        </div>
            ...
            {filteredProducts.length === 0 && (
              <div className="text-center py-16 bg-[#111113] border border-[#27272a] rounded-xl">
                <div className="w-16 h-16 bg-[#18181b] rounded-full flex items-center justify-center mx-auto mb-4">
                  <Cpu className="w-8 h-8 text-[#71717a]" />
                </div>
                <p className="text-[#71717a] font-mono text-sm">NO_PRODUCTS_FOUND</p>
                <p className="text-[#52525b] text-xs mt-1">Try adjusting your filters</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-[#111113] border border-[#27272a] rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in">
            <div className="relative">
              <button
                ...
                className="absolute top-4 right-4 z-10 w-10 h-10 bg-[#18181b] border border-[#27272a] rounded-lg flex items-center justify-center text-[#71717a] hover:text-white hover:border-[#3b82f6] transition-all"
              >
                <X className="w-5 h-5" />
              </button>
              <div className="aspect-video bg-[#18181b]">
                <img
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-sm text-[#71717a] font-mono">{selectedProduct.rating} ({selectedProduct.reviews} reviews)</span>
              </div>
              ...
              {/* Specifications */}
              <div className="mb-6">
                <h3 className="text-xs font-mono text-[#71717a] uppercase tracking-wider mb-3">Specifications</h3>
                <div className="flex flex-wrap gap-2">
                  {selectedProduct.specs.map((spec, i) => (
                    <span key={i} className="px-3 py-1.5 bg-[#18181b] border border-[#27272a] text-[#a1a1aa] text-sm rounded-lg font-mono">
                      {spec}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-2 mb-6">
                <span className={cn(
                  ...
                  selectedProduct.stock > 10 ? "bg-[#22c55e]" : selectedProduct.stock > 0 ? "bg-[#f59e0b]" : "bg-[#ef4444]"
                )} />
              ...
              <div className="flex items-center justify-between pt-4 border-t border-[#27272a]">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-[#3b82f6] text-white font-semibold rounded-lg hover:bg-[#2563eb] transition-all hover:shadow-[0_0_20px_rgba(59,130,246,0.4)]"
                >
